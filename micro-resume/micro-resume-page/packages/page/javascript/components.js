...
Vue.component('cv-content', {
  template: '<div :class=\'{grid:true}\'><slot></slot></div>'
});
  ...
  template: `
	<div class='section'>
		...
		<h2 class="header float-in-up">Experience&nbsp;
			<span>  
				<div id="typewriter" >
					<span class="cursor-c"> |</span>
				</div>
			</span>
		</h2>
		<span class="sub-section expand" @click="toggleOpen()">Expand</span>
		<div v-for='job in data' class='sub-section'>
			...
			<p class="sub-header">{{job.description}}</p>
			<ul class="sub-section" v-if="expand" :style='{ height: 2 + "rem" }'><li v-for="r in job.results">{{r}}</li></ul>
			<ul class="sub-section" v-if="!expand"><li v-for="r in job.results">{{r}}</li></ul>
  ...
  template: `
	<div class='section'>
		...
		<div v-for='project in data' class='sub-section'>
			...
			<p class="sub-header">{{project.description}}</p>
			<div class="sub-section">
				<h4 class="sub-header">KEY FEATURES</h4>
				<ul class="results-list"><li v-for="r in project.features"><span>{{r}}</span></li></ul>
			</div>
			<div class="sub-section">
				<medium style="display: inline-block" v-if="project.technologies">TECH USED : </medium>
				<small v-for="tech in project.technologies" class="tag">{{tech}}</small>
			</div>
		</div>
	</div>`
});
  ...
  template: `
	<div class='section'>
		...
		<div v-for='degree in data' class='sub-section'>
			...
			<p class="sub-header">{{degree.summary}}</p>
			<div class="sub-section">
  ...
  template: `
    <div class='section'>
		...
		<div class="sub-section" v-for="skills in data" :key="skills">
        	<h3 class="header">{{skills.title}} 
				<span v-if="skills.level" class="proficiency-level animated-underline"><span class="proficiency-bar" :style="{width:(skills.level*20)+'%'}"></span>
				</span>
			</h3>
  ...
  template: `
	<div @click="toggleOpen()" :class="{circle:true, open:open, openable:true}" :style="{height:size + 'px', width:size + 'px', fontSize: (size/25) + 'em'}"> 
		...
		<div  v-if="open" class="circle-icon-info">
			...
			<h5>V {{info.appVersion}}</h5>
			<h5>CV Version</h5> 
			<h5>V {{info.version}}</h5>
		</div>
	</div>`,
...
Vue.component('letter-content', {
  template: '<div :class=\'{grid:true}\'><slot></slot></div>'
});

Vue.component('letter-section', {
  props: [ 'letter' ],
  template: `
	<div class="section">
		<div class="sub-section">
...
Vue.component('candid-section', {
  props: [ 'letter' ],
  template: `
	<div class="section">
		<h2 class="header">{{letter.candidature}}</h2>
	</div>
  `
});
